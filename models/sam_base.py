...
def patch_sam_for_img_size(sam: nn.Module, img_size: int) -> None:
    ...
    if hasattr(sam, "image_encoder"):
        ...
        if hasattr(sam.image_encoder, "pos_embed"):
            original_pos_embed = sam.image_encoder.pos_embed
            # 计算原始网格尺寸。
            # 官方 segment-anything 的 pos_embed 形状是 [1, H, W, D]；
            # 一些测试桩或其他 ViT 实现可能使用 [1, N, D]。
            # 这里同时兼容两种格式，避免位置编码插值后和 image encoder 输出 shape 不匹配。
            if original_pos_embed.ndim == 4:
                old_size = int(original_pos_embed.shape[1])
                pos_format = "bhwd"
            elif original_pos_embed.ndim == 3:
                old_size = int(original_pos_embed.shape[1] ** 0.5)
                pos_format = "bnd"
            else:
                raise ValueError(
                    "不支持的位置编码形状: "
                    f"{tuple(original_pos_embed.shape)}，期望 [1,H,W,D] 或 [1,N,D]"
                )
            ...
            if old_size != new_size:
                ...
                # 步骤 1: 统一转换为图像格式 [1,D,H,W] 以便插值。
                # - 官方格式 [1,H,W,D] 直接 permute；
                # - 扁平格式 [1,N,D] 先还原为 [1,H,W,D]。
                if pos_format == "bnd":
                ...
                # 步骤 3: 转回原始格式。
                # shape 变化：
                # - 官方格式: [1,D,H',W'] -> [1,H',W',D]
                # - 扁平格式: [1,D,H',W'] -> [1,H',W',D] -> [1,N',D]
                ...
                # 步骤 4: 更新模型的位置编码。
                # 输入输出说明：
                # - 输入 original_pos_embed 可能是 nn.Parameter，形状为 [1,H,W,D] 或 [1,N,D]。
                # - 输出 pos_embed 是插值后的新位置编码，保持原始维度格式。
                # 为什么不能直接赋 Tensor：
                # - 官方 SAM 的 pos_embed 是 nn.Parameter，PyTorch 2.11 会禁止把普通 Tensor
                #   直接赋给 Parameter 字段，否则训练前构建模型会报 TypeError。
                # - 因此这里保留原字段类型和 requires_grad 属性，保证 state_dict 与训练流程兼容。
                if isinstance(original_pos_embed, nn.Parameter):
                    sam.image_encoder.pos_embed = nn.Parameter(
                        pos_embed,
                        requires_grad=original_pos_embed.requires_grad,
                    )
                else:
    ...
    if hasattr(sam, "prompt_encoder"):
        new_size = img_size // 16
        # SAM 的 prompt_encoder 会用 image_embedding_size 生成 dense prompt embedding。
        # 当 image_encoder 被改成 512 输入后，图像特征从 [B,256,64,64] 变为 [B,256,32,32]；
        # 如果这里仍保留旧的 (64,64)，mask_decoder 内部执行
        # image_embeddings + dense_prompt_embeddings 时会发生 shape mismatch。
        # 因此同步更新：
        # - image_embedding_size: dense prompt 输出空间尺寸，输出应为 [B,256,new_size,new_size]
        # - input_image_size: prompt 坐标归一化参考尺寸，保持和训练图像尺寸一致
        # - mask_input_size: mask prompt 输入尺寸，SAM 约定为 image_embedding_size 的 4 倍
        if hasattr(sam.prompt_encoder, "image_embedding_size"):
            sam.prompt_encoder.image_embedding_size = (new_size, new_size)
        if hasattr(sam.prompt_encoder, "input_image_size"):
            sam.prompt_encoder.input_image_size = (img_size, img_size)
        if hasattr(sam.prompt_encoder, "mask_input_size"):
            sam.prompt_encoder.mask_input_size = (new_size * 4, new_size * 4)
