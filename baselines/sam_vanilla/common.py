...
def load_gt_mask_binary(path: Path) -> np.ndarray:
    """读取 GT mask 并转为“裂缝=1、背景=0”的 0/1 二值图。
    ...
    为什么这样做:
    - SAM-Box-Oracle 会用 GT mask 生成 box prompt；如果极性反了，box 会框住背景。
    - 这里复用统一的 normalize_crack_mask，确保 baseline 指标与训练目标一致。
    """
    m = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE)
    if m is None:
        raise RuntimeError(f"标注读取失败: {path}")
    return normalize_crack_mask(m)
